...
public:
  int snakesAndLadders(vector<vector<int>> &board) {
    ...
    // The rest is just performing BFS, but with special conditions that we need
    // to fulfil For BFS, we can either - traverse a ladder or snake which we
    // check first, otherwise, we check curr + 1 -> curr + 6 and add them to the
    // queue
    int n = board.size();
